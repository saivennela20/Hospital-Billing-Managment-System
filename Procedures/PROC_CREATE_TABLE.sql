          ...
          SELECT count(*) into row_count FROM user_tables where table_name = UPPER(p_tabname);
           ...
           IF(row_count > 0)
            THEN
                ...
                drop_stmt:= 'DROP TABLE ' || v_tab_name || ' CASCADE CONSTRAINTS PURGE'; 
            EXECUTE IMMEDIATE drop_stmt;
             EXECUTE IMMEDIATE v_tab_script;
             DBMS_OUTPUT.PUT_LINE('Table droped and recreated'); 
            ELSE
                DBMS_OUTPUT.PUT_LINE('--------------------------');  
                EXECUTE IMMEDIATE v_tab_script;
